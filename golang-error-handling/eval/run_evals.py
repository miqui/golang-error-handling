#!/usr/bin/env python3
"""Eval runner for the golang-error-handling agent skill."""
...
def _first_error_string_capitalized(code: str) -> bool:
    literals = re.findall(
        r'(?:errors\.New|fmt\.Errorf|oops\.\w+)\s*\(\s*"([^"]+)"',
        code,
    )
    return any(
        lit.split("%")[0].strip() and lit.split("%")[0].strip()[0].isupper()
        for lit in literals
    )
...
def check_deterministic(assertion_text: str, code: str) -> Optional[bool]:
    ...
    if "uses slog" in a and "not log.printf" in a:
        return bool(re.search(r"\bslog\b", code)) and not bool(
            re.search(r"\blog\.Printf\b", code)
        )
    ...
    if "errors.new" in a and ("sentinel" in a or "fmt.errorf" in a):
        return bool(re.search(r"\berrors\.New\b", code)) and not bool(
            re.search(r"\bvar\s+Err\w+\s*=\s*fmt\.Errorf", code)
        )
    ...
    if "%w" in a and "not" in a and (
        "boundary" in a or "translated" in a or "break" in a
    ):
...
_JUDGE_SYSTEM = (
    "You are a precise Go code reviewer evaluating one assertion about a code "
    "response. Reply with EXACTLY one of: PASS: <one-sentence reason> or "
...
def judge_assertion(
    ...
    user = (
        f"## Assertion\n"
        f"ID: {assertion['id']}\n"
        f"Text: {assertion['text']}\n\n"
        f"## Go code to evaluate\n```go\n{response}\n```\n\n"
        f"Does the code satisfy this assertion?"
    )
    raw = call_model(client, judge_model, _JUDGE_SYSTEM, user, max_tokens=300).strip()
    passed = raw.upper().startswith("PASS")
    match = re.match(r"(?:PASS|FAIL):\s*(.*)", raw, re.IGNORECASE)
    ...
    for assertion in eval_case["assertions"]:
        ...
        if det is not None:
            assertion_result = AssertionResult(
                ...
                reason="regex/AST check",
            )
        else:
            assertion_result = judge_assertion(
                client,
                judge_model,
                response,
                assertion,
        ...
        result.passed += int(assertion_result.passed)
        result.failed += int(not assertion_result.passed)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run evals for the golang-error-handling skill."
    )
    parser.add_argument("--skill", default="skill.md")
    parser.add_argument("--evals", default="eval/eval.json")
